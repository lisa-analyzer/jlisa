...
public class MethodOverloadingTest extends JLiSAAnalysisExecutor {
	...
	public void testMethodOverloading7() throws IOException {
		...
		// NOTE: Test7.java is not a valid Java class because `String...` is
		// just syntactic sugar for `String[]`.
		// This means the methods foo(String... args) and foo(String[] args)
		// actually have the same signature,
		// so the frontend should raise a ParsingException.
		// Currently, since varargs are not supported, `String...` is treated
		// simply as `String`.
		// To make this test pass for now, we assume that no
		// duplicated_method_descriptor exception
