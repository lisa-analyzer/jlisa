...
public class PopulateUnitsASTVisitor extends ScopedVisitor<UnitScope> {
    ...
    public PopulateUnitsASTVisitor(ParsingEnvironment env, UnitScope scope) {
        super(env, scope);
    }
    ...
    private boolean buildClassUnit(
        ...
        name = (getScope().getPackage().isEmpty() ? "" : scope.getPackage() + ".") + (outer == null ? "" : outer + ".") + typeDecl.getName().toString();
        if (!processed.add(name))
            return false;
