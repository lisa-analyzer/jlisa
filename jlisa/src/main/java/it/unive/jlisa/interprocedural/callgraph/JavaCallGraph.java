 ...
 * method signatures, parameter types, and type hierarchies. If multiple
 * equally good matches are found, a {@link CallResolutionException} is thrown.
 * </p>
 */
public abstract class JavaCallGraph extends BaseCallGraph {
     ...
     * <ol>
     *   <li>Looks up the {@link CompilationUnit} corresponding to the
     *       call's qualifier (e.g., the class in which the method is declared).</li>
     *   <li>Traverses the class hierarchy starting from that unit using
     *       the program's {@link HierarcyTraversalStrategy}.</li>
     *   <li>For each candidate {@link CodeMember}, checks whether it is a valid
     *       target using {@link #isATarget(UnresolvedCall, SymbolAliasing, CodeMember, Set[], boolean)}.</li>
     ...
     *       (perfect match) is found, resolution stops immediately and that target
     *       is returned.</li>
     *   <li>If multiple candidates have the same best distance greater than 0,
     *       a {@link CallResolutionException} is thrown due to ambiguity.</li>
     * </ol>
     * </p>
     *
     * @param call     the unresolved call to resolve
     * @param types    the possible runtime types of the call's parameters
     * @param targets  the collection where resolved {@link CFG} targets will be stored
     * @param natives  the collection where resolved {@link NativeCFG} targets will be stored
     * @param aliasing aliasing information that may affect resolution
     *
     * @throws CallResolutionException if multiple equally suitable targets are found
     */
    @Override
    public void resolveNonInstance(
        ...
        for (CompilationUnit cu : strategy.traverse(call, targetUnit)) {
            ...
            for (CodeMember cm : cu.getCodeMembers()) {
                if (!isATarget(call, aliasing, cm, types, false)) continue;
                int distance = distanceFromPerfectTarget(call, cm, false);
                if (distance < 0) continue; // incomparable
    ...
    /**
     * Resolves an instance call (a method call invoked on an object) to its possible targets.
     * <p>
     * The resolution process works as follows:
     * <ol>
     ...
     *   <li>Determines the possible runtime {@link Type}s of the receiver expression,
     *       and retrieves the corresponding {@link CompilationUnit}s using
     *       {@link #getReceiverCompilationUnit(Type)}.</li>
     *   <li>For each receiver type, traverses the class hierarchy of its unit using
     *       the program's {@link HierarcyTraversalStrategy}.</li>
     *   <li>For each candidate {@link CodeMember}, checks whether it is a valid
     *       instance target using {@link #isATarget(UnresolvedCall, SymbolAliasing, CodeMember, Set[], boolean)}.</li>
     *   <li>Computes the "distance" of the candidate from a perfect match using
     *       {@link #distanceFromPerfectTarget(UnresolvedCall, CodeMember, boolean)}.</li>
     *   <li>Keeps track of the best match (lowest distance). If a distance of 0
     *       (perfect match) is found, resolution for that receiver stops immediately.</li>
     *   <li>If multiple candidates have the same best distance greater than 0,
     *       a {@link CallResolutionException} is thrown due to ambiguity.</li>
     ...
     * @param aliasing aliasing information that may affect resolution
     *
     * @throws CallResolutionException if the call has no receiver, or if multiple equally suitable targets are found
     */
    @Override
        ...
        for (Type recType : getPossibleTypesOfReceiver(receiver, types[0])) {
            ...
            for (CompilationUnit cu : strategy.traverse(call, unit)) {
                ...
                for (CodeMember cm : cu.getInstanceCodeMembers(false)) {
                    if (!isATarget(call, aliasing, cm, types, true)) continue;
     ...
     *
     * @return the compilation unit of the receiver, or {@code null} if unavailable
     */
    ...
    /**
     * Adds a resolved code member to the appropriate collection based on whether
     * it is a {@link CFG} or a {@link NativeCFG}.
     *
     ...
     *
     * @return {@code true} if the code member is a valid target, {@code false} otherwise
     */
     ...
     *
     * @return {@code true} if the call matches an alias, {@code false} otherwise
     */
    private boolean matchesAlias(UnresolvedCall call, SymbolAliasing aliasing,
    ...
    /**
     * Computes the "distance" between a candidate code member and a perfect match
     * for a call.
     * <p>
     * Distance reflects type compatibility: zero means an exact match,
     * higher values mean progressively less exact matches, and negative
     * means incomparable.
     * </p>
     *
     * @param call     the unresolved call
        ...
        for (int i = startIdx; i < params.length; i++) {
            ...
            if (paramType instanceof JavaNumericType numericParam) {
                if (formalType instanceof JavaNumericType numericFormal) {
                    ...
                    if (paramDist < 0) return -1; // incomparable
                    distance += paramDist;
                } else {
                    return -1;
                }
