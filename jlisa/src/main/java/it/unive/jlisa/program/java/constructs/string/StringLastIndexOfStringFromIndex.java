...
public class StringLastIndexOfStringFromIndex extends TernaryExpression implements PluggableStatement {
	protected Statement originating;

	protected StringLastIndexOfStringFromIndex(CFG cfg, CodeLocation location, Expression left, Expression middle,
			Expression right) {
		super(cfg, location, "replaceAll", left, middle, right);
	}

	public static StringLastIndexOfStringFromIndex build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringLastIndexOfStringFromIndex(cfg, location, params[0], params[1], params[2]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdTernarySemantics(
		...
		it.unive.lisa.symbolic.value.TernaryExpression lastIndexOf = new it.unive.lisa.symbolic.value.TernaryExpression(
				getProgram().getTypes().getStringType(), 
				accessLeft,
				accessMiddle,
				right, 
				JavaStringLastIndexOfStringFromIndexOperator.INSTANCE, 
				getLocation());
