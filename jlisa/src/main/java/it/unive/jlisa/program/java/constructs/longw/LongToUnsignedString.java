...
import it.unive.jlisa.program.operator.JavaLongToUnsignedStringOperator;
...
public class LongToUnsignedString extends UnaryExpression implements PluggableStatement {
	protected Statement originating;

	public LongToUnsignedString(
			CFG cfg,
			CodeLocation location,
			Expression exp) {
		super(cfg, location, "toUnsignedString", exp);
	}

	public static LongToUnsignedString build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongToUnsignedString(cfg, location, params[0]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdUnarySemantics(
		...
		it.unive.lisa.symbolic.value.UnaryExpression toString = new it.unive.lisa.symbolic.value.UnaryExpression(
				stringType,
				accessExpr,
				JavaLongToUnsignedStringOperator.INSTANCE,
				getLocation());
