...
public class StringToUpperCase extends UnaryExpression implements PluggableStatement {
	protected Statement originating;

	public StringToUpperCase(CFG cfg, CodeLocation location, Expression exp) {
		super(cfg, location, "toUpperCase", exp);
	}

	public static StringToUpperCase build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringToUpperCase(cfg, location, params[0]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdUnarySemantics(
		...
		it.unive.lisa.symbolic.value.UnaryExpression upper = new it.unive.lisa.symbolic.value.UnaryExpression(stringType, accessExpr, JavaStringToUpperCaseOperator.INSTANCE, getLocation());
