...
public class CharacterDigit extends BinaryExpression implements PluggableStatement {
	protected Statement originating;

	public CharacterDigit(CFG cfg, CodeLocation location, Expression left, Expression right) {
		super(cfg, location, "digit", left, right);
	}

	public static CharacterDigit build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new CharacterDigit(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		it.unive.lisa.symbolic.value.BinaryExpression equalsExpr = new it.unive.lisa.symbolic.value.BinaryExpression(
				...
				left, 
				right, 
				JavaCharacterDigitOperator.INSTANCE, 
				getLocation());
