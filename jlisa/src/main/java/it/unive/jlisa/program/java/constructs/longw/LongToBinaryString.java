...
public class LongToBinaryString extends UnaryExpression implements PluggableStatement {
	protected Statement originating;

	public LongToBinaryString(
			CFG cfg,
			CodeLocation location,
			Expression exp) {
		super(cfg, location, "toBinaryString", exp);
	}

	public static LongToBinaryString build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongToBinaryString(cfg, location, params[0]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdUnarySemantics(
		...
		it.unive.lisa.symbolic.value.UnaryExpression toString = new it.unive.lisa.symbolic.value.UnaryExpression(
				stringType,
				accessExpr,
				JavaLongToBinaryStringOperator.INSTANCE,
				getLocation());
