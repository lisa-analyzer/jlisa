...
import it.unive.jlisa.program.operator.JavaStrictMathMin;
...
public class StrictMathMin extends it.unive.lisa.program.cfg.statement.BinaryExpression implements PluggableStatement {
	protected Statement originating;

	public StrictMathMin(
			CFG cfg,
			CodeLocation location,
			Expression left,
			Expression right) {
		super(cfg, location, "strictmin", left, right);
	}

	public static StrictMathMin build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StrictMathMin(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		for (Type lType : leftTypes) {
			for (Type rType : rightTypes) {
				...
				BinaryExpression min = new BinaryExpression(
						JavaDoubleType.INSTANCE,
						leftExpr,
						rightExpr,
						JavaStrictMathMin.INSTANCE,
						getLocation());
				result = result.lub(analysis.smallStepSemantics(state, min, originating));
			}
		}
