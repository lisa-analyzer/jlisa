...
import it.unive.jlisa.program.operator.JavaIntToOctalStringOperator;
...
public class IntegerToOctalString extends UnaryExpression implements PluggableStatement {
	protected Statement originating;

	public IntegerToOctalString(
			CFG cfg,
			CodeLocation location,
			Expression expr) {
		super(cfg, location, "toOctalString", expr);
	}

	public static IntegerToOctalString build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new IntegerToOctalString(cfg, location, params[0]);
	}
	...
	public <A extends AbstractLattice<A>,
		...
		it.unive.lisa.symbolic.value.UnaryExpression lower = new it.unive.lisa.symbolic.value.UnaryExpression(
				stringType, access, JavaIntToOctalStringOperator.INSTANCE, getLocation());
