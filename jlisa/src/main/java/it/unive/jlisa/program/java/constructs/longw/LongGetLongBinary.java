...
import it.unive.jlisa.program.operator.JavaLongGetLongBinaryOperator;
...
public class LongGetLongBinary extends BinaryExpression implements PluggableStatement {
	protected Statement originating;

	public LongGetLongBinary(
			CFG cfg,
			CodeLocation location,
			Expression left,
			Expression right) {
		super(cfg, location, "getLong", left, right);
	}

	public static LongGetLongBinary build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongGetLongBinary(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		it.unive.lisa.symbolic.value.BinaryExpression toString = new it.unive.lisa.symbolic.value.BinaryExpression(
				longWrapperType,
				accessLeft,
				accessRight,
				JavaLongGetLongBinaryOperator.INSTANCE,
				getLocation());
		...
		for (SymbolicExpression ref : callState.getExecutionExpressions()) {
			AccessChild access = new AccessChild(longWrapperType, ref, var, getLocation());
			AnalysisState<A> sem = analysis.assign(callState, access, toString, this);
			tmp = tmp.lub(sem);
		}
