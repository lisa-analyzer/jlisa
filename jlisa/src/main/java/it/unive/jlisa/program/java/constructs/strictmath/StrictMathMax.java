...
import it.unive.jlisa.program.operator.JavaStrictMathMax;
...
public class StrictMathMax extends it.unive.lisa.program.cfg.statement.BinaryExpression implements PluggableStatement {
	protected Statement originating;

	public StrictMathMax(
			CFG cfg,
			CodeLocation location,
			Expression left,
			Expression right) {
		super(cfg, location, "strictmax", left, right);
	}

	public static StrictMathMax build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StrictMathMax(cfg, location, params[0], params[1]);
	}
