...
import it.unive.jlisa.program.operator.JavaLongRotateLeftOperator;
...
public class LongRotateLeft extends BinaryExpression implements PluggableStatement {
	protected Statement originating;

	protected LongRotateLeft(
			CFG cfg,
			CodeLocation location,
			Expression left,
			Expression right) {
		super(cfg, location, "rotateLeft", left, right);
	}

	public static LongRotateLeft build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongRotateLeft(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		it.unive.lisa.symbolic.value.BinaryExpression rotate = new it.unive.lisa.symbolic.value.BinaryExpression(
				...
				accessLeft,
				accessRight,
				JavaLongRotateLeftOperator.INSTANCE,
				getLocation());
