...
public class LongSum extends BinaryExpression implements PluggableStatement {
	...
	protected LongSum(
			...
			Expression left,
			Expression right) {
		super(cfg, location, "sum", left, right);
	}

	public static LongSum build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongSum(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		it.unive.lisa.symbolic.value.BinaryExpression sum = new it.unive.lisa.symbolic.value.BinaryExpression(
				JavaLongType.INSTANCE,
				accessLeft,
				accessRight,
				JavaLongSumOperator.INSTANCE,
				getLocation());

		return analysis.smallStepSemantics(state, sum, originating);
	}
