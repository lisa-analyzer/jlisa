...
public class LongToHexString extends UnaryExpression implements PluggableStatement {
	protected Statement originating;

	public LongToHexString(
			CFG cfg,
			CodeLocation location,
			Expression exp) {
		super(cfg, location, "toHexString", exp);
	}

	public static LongToHexString build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongToHexString(cfg, location, params[0]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdUnarySemantics(
		...
		it.unive.lisa.symbolic.value.UnaryExpression toString = new it.unive.lisa.symbolic.value.UnaryExpression(
				stringType,
				accessExpr,
				JavaLongToHexStringOperator.INSTANCE,
				getLocation());
