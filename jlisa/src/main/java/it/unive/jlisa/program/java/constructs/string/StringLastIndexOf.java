...
public class StringLastIndexOf  extends BinaryExpression implements PluggableStatement {
	protected Statement originating;

	public StringLastIndexOf(CFG cfg, CodeLocation location, Expression left, Expression right) {
		super(cfg, location, "lastIndexOf", left, right);
	}

	public static StringLastIndexOf build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringLastIndexOf(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		it.unive.lisa.symbolic.value.BinaryExpression lastIndexOf = new it.unive.lisa.symbolic.value.BinaryExpression(
				...
				accessLeft, 
				right, 
				JavaStringLastIndexOfOperator.INSTANCE, 
				getLocation());
