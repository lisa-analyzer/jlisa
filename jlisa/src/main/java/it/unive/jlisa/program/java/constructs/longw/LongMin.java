...
public class LongMin extends BinaryExpression implements PluggableStatement {
	...
	public static LongMin build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongMin(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		it.unive.lisa.symbolic.value.BinaryExpression min = new it.unive.lisa.symbolic.value.BinaryExpression(
				JavaLongType.INSTANCE,
				accessLeft,
				accessRight,
				JavaLongMinOperator.INSTANCE,
				getLocation());

		return analysis.smallStepSemantics(state, min, originating);
	}
