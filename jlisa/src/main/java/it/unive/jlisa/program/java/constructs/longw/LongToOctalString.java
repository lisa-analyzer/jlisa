...
import it.unive.jlisa.program.operator.JavaLongToOctalStringOperator;
...
public class LongToOctalString extends UnaryExpression implements PluggableStatement {
	protected Statement originating;

	public LongToOctalString(
			CFG cfg,
			CodeLocation location,
			Expression exp) {
		super(cfg, location, "toOctalString", exp);
	}

	public static LongToOctalString build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongToOctalString(cfg, location, params[0]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdUnarySemantics(
		...
		it.unive.lisa.symbolic.value.UnaryExpression toString = new it.unive.lisa.symbolic.value.UnaryExpression(
				stringType,
				accessExpr,
				JavaLongToOctalStringOperator.INSTANCE,
				getLocation());
