...
public class StringReplaceFirst extends TernaryExpression implements PluggableStatement {
	...
	protected StringReplaceFirst(
			...
			Expression middle,
			Expression right) {
		super(cfg, location, "replaceFirst", left, middle, right);
	}

	public static StringReplaceFirst build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringReplaceFirst(cfg, location, params[0], params[1], params[2]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdTernarySemantics(
		...
		it.unive.lisa.symbolic.value.TernaryExpression replaceAll = new it.unive.lisa.symbolic.value.TernaryExpression(
				...
				accessMiddle,
				accessRight,
				JavaStringReplaceFirstOperator.INSTANCE,
				getLocation());
