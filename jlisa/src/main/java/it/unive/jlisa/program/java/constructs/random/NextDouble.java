...
public  class NextDouble extends UnaryExpression implements PluggableStatement {
	...
	public static NextDouble build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new NextDouble(cfg, location, params[0]);
	}
	...
	public <A extends AbstractState<A>> AnalysisState<A> fwdUnarySemantics(InterproceduralAnalysis<A> interprocedural, AnalysisState<A> state, SymbolicExpression expr, StatementStore<A> expressions) throws SemanticException {
		return state.smallStepSemantics(new PushAny(JavaDoubleType.INSTANCE, getLocation()), originating);
	}
