...
public class LongCompareUnsigned extends BinaryExpression implements PluggableStatement {
	protected Statement originating;

	protected LongCompareUnsigned(
			CFG cfg,
			CodeLocation location,
			Expression left,
			Expression right) {
		super(cfg, location, "long-compare-U", left, right);
	}

	public static LongCompareUnsigned build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new LongCompareUnsigned(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		it.unive.lisa.symbolic.value.BinaryExpression rotate = new it.unive.lisa.symbolic.value.BinaryExpression(
				...
				accessLeft,
				accessRight,
				JavaLongCompareUnsignedOperator.INSTANCE,
				getLocation());
