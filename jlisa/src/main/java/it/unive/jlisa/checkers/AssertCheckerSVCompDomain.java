 ...
 */
public class AssertCheckerSVCompDomain
		implements
		SemanticCheck<
				SimpleAbstractState<
						HeapEnvironment<AllocationSites>,
						ValueEnvironment<SVCompDomainValue>,
	...
	private void checkRuntimeException(
			CheckToolWithAnalysisResults<
					SimpleAbstractState<HeapEnvironment<AllocationSites>, ValueEnvironment<SVCompDomainValue>,
							TypeEnvironment<TypeSet>>,
					SimpleAbstractDomain<HeapEnvironment<AllocationSites>, ValueEnvironment<SVCompDomainValue>,
							TypeEnvironment<TypeSet>>> tool,
			CFG graph,
			Statement node)
	...
	private void checkAssert(
		...
		for (var result : tool.getResultOf(graph)) {
			...
			for (SymbolicExpression expr : state.getExecutionExpressions()) {
				ValueEnvironment<SVCompDomainValue> valueState = state.getExecutionState().valueState;
