...
import it.unive.jlisa.analysis.ConstantPropWithIntervalLattice;
...
public class AssertCheckerCPInterval
		implements
		SemanticCheck<
				SimpleAbstractState<
						HeapEnvironment<AllocationSites>,
						ConstantPropWithIntervalLattice,
						TypeEnvironment<TypeSet>>,
				SimpleAbstractDomain<
						HeapEnvironment<AllocationSites>,
						ConstantPropWithIntervalLattice,
						TypeEnvironment<TypeSet>>> {
	private static final Logger LOG = LogManager.getLogger(AssertCheckerCPInterval.class);
	...
	private void checkRuntimeException(
		...
		for (var result : tool.getResultOf(graph)) {
			...
			SimpleAbstractState<HeapEnvironment<AllocationSites>, ConstantPropWithIntervalLattice,
	...
	private void checkAssert(
		...
		for (var result : tool.getResultOf(graph)) {
			...
			for (SymbolicExpression expr : state.getExecutionExpressions()) {
				...
				if (!state.getExecutionState().valueState.isBottom()) {
					if (!state.getExecutionState().valueState.isTop()) {
